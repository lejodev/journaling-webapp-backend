...
export class JournalService {
  ...
  async update(id: number, updateJournalDto: UpdateJournalDto) {
    try {
      ...
      if (
        !updateJournalDto ||
        (!updateJournalDto.content && !updateJournalDto.title)
      ) {
        throw new BadRequestException(
          `At least a title or content should be provided`,
        );
      }

      const journal = await firstValueFrom(
        this.wrapperService.findOne(Journal, { id }),
      );
      console.log(journal);
      ...
      const result = await firstValueFrom(
        this.wrapperService.update(Journal, { id }, updateJournalDto),
      );

      if (result.affected == 0) {
        throw new NotFoundException(
          `Journal with id: ${id} can not be updated`,
        );
      }
  ...
  async remove(id: number) {
    ...
    const journal = await firstValueFrom(
      this.wrapperService.findOne(Journal, { id: id }),
    );
    console.log(journal);
    ...
    const deleteResult = await firstValueFrom(
      this.wrapperService.delete(Journal, id.toString()),
    );

    if (deleteResult.affected == 0) {
      throw new NotFoundException(
        `Journal with id: ${id} could not be deleted`,
      );
    }
